...
class rpc:
    ...
    def _set_packet(self, magic_value, payload=bytes()):  # private
        ...
        try:
            ...
            new_payload[2:-2] = payload.encode()
    ...
    def stream_writer(self, call_back, write_timeout_ms=5000):  # public
        ...
        while True:
            ...
            if credits > 0:
                data = call_back()
                try:
                    self._stream_put_bytes(self._set_packet(0x542E, struct.pack("<I", len(data))), 1000)
                except OSError:
                    return
                try:
...
class rpc_master(rpc):
    ...
    def __put_command(self, command, data, timeout):  # private
        ...
        while (int(time.time() * 1000) - start) < timeout:
            ...
            if self._get_packet(self._COMMAND_HEADER_PACKET_MAGIC, self.__in_command_header_buf,
                                self._get_short_timeout) is not None:
                self.put_bytes(out_data, self._put_long_timeout)
                if self._get_packet(self._COMMAND_DATA_PACKET_MAGIC, self.__in_command_data_buf,
                                    self._get_short_timeout) is not None:
                    return True
    ...
    def __get_result(self, timeout):  # private
        ...
        while (int(time.time() * 1000) - start) < timeout:
            ...
            packet = self._get_packet(self._RESULT_HEADER_PACKET_MAGIC, self.__in_result_header_buf,
            ...
            if packet is not None:
                ...
                dat_packet = self._get_packet(self._RESULT_DATA_PACKET_MAGIC, in_result_data_buf,
    ...
    def call(self, name, data=bytes(), send_timeout=1000, recv_timeout=1000):  # public
        return self.__get_result(recv_timeout) if self.__put_command(self._hash(name, len(name)), data,
                                                                     send_timeout) else None
...
class rpc_slave(rpc):
    ...
    def __get_command(self, timeout):  # private
        ...
        while (int(time.time() * 1000) - start) < timeout:
            ...
            self._zero(self.__in_command_header_buf[0], len(self.__in_command_header_buf[0]))
            self._flush()
            packet = self._get_packet(self._COMMAND_HEADER_PACKET_MAGIC, self.__in_command_header_buf,
                                      self._get_short_timeout)
            if packet is not None:
                ...
                dat_packet = self._get_packet(self._COMMAND_DATA_PACKET_MAGIC, in_command_data_buf,
                                              self._get_long_timeout)
                if dat_packet is not None:
    ...
    def __put_result(self, data, timeout):  # private
        ...
        while (int(time.time() * 1000) - start) < timeout:
            ...
            self._flush()
            if self._get_packet(self._RESULT_HEADER_PACKET_MAGIC, self.__in_response_header_buf,
                                self._get_short_timeout) is not None:
                self.put_bytes(out_header, self._put_short_timeout)
                if self._get_packet(self._RESULT_DATA_PACKET_MAGIC, self.__in_response_data_buf,
                                    self._get_short_timeout) is not None:
                    self.put_bytes(out_data, self._put_long_timeout)
                    return True
            # Avoid timeout livelocking.
    ...
    def loop(self, recv_timeout=1000, send_timeout=1000):  # public
        while True:
            ...
            if command is not None:
                ...
                if self.__put_result(cb(data) if cb is not None else bytes(),
                                     send_timeout) and self.__schedule_cb is not None:
                    self.__schedule_cb()
                self.__schedule_cb = None
...
class rpc_kvarser_can_master(rpc_master):
    ...
    def get_bytes(self, buff, timeout_ms):  # protected
        ...
        for i in range(0, l, 8):
            ...
            try:
                ...
                if frame.id == self.__message_id and frame.dlc == expected:
                    buff[i:i + 8] = frame.data
                else:
    ...
    def put_bytes(self, data, timeout_ms):  # protected
        ...
        for i in range(0, len(view), 8):
            try:
                self.__can.writeWait(Frame(id_=self.__message_id, data=view[i:i + 8]), timeout=self._put_short_timeout)
            except canlib.CanError:
                break
